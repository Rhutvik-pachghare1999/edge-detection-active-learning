...
def test_build_subset_sizes_and_disjointness():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        manifest = fetch.build_subset(
            coco_root, tmp_path, subset_size=10, train_size=7, subset_name="subset"
        )
        ...
        assert (out_root / "labels" / "train").exists()
        assert (out_root / "images" / "val").exists()
...
def test_build_subset_sorted_by_coco_id():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        coco_root = _make_synthetic_coco(tmp_path, n_images=10)
        manifest = fetch.build_subset(
            coco_root, tmp_path, subset_size=8, train_size=5, subset_name="subset"
        )
...
def test_build_subset_4k_sizes_and_disjointness():
    """A distinct subset_4k directory can be built with disjoint train/test."""
    fetch = _load_fetch_module()
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        coco_root = _make_synthetic_coco(tmp_path, n_images=20)
        manifest = fetch.build_subset(
            coco_root,
            tmp_path,
            subset_size=20,
        ...
        out_root = tmp_path / "subset_4k"

        assert manifest["subset_size"] == 20
        assert manifest["train_size"] == 15
        assert manifest["test_size"] == 5
        assert (out_root / "labels" / "train").exists()
        assert (out_root / "images" / "val").exists()
...
def test_build_subset_yolo_class_mapping():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        coco_root = _make_synthetic_coco(tmp_path, n_images=10)
        out_root = tmp_path / "subset"
        fetch.build_subset(coco_root, tmp_path, subset_size=10, train_size=5, subset_name="subset")
...
def test_build_subset_manifest_has_checksums_and_relative_paths():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        coco_root = _make_synthetic_coco(tmp_path, n_images=10)
        manifest = fetch.build_subset(
            coco_root, tmp_path, subset_size=10, train_size=5, subset_name="subset"
        )
...
def test_build_subset_rejects_missing_annotation():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        with pytest.raises(FileNotFoundError):
            fetch.build_subset(Path(tmp), Path(tmp), subset_size=10, train_size=5, subset_name="subset")
